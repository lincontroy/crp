...
class MoneyTransferController extends Controller
{
    ...
    public function submit(Request $request) {
        ...
        if(!$sender_wallet) return Response::error(['Your wallet isn\'t available with currency ('.$default_currency->code.')'],[],404);

        $receiver_currency = Currency::receiver()->active()->where('code',$default_currency->code)->first();
        if(!$receiver_currency) return Response::error(['Currency ('.$validated['receiver_currency'].') isn\'t available for receive any transaction'],[],404);
        ...
        return Response::success(['Successfully money transfer to @'.$receiver_wallet->user->username.' success'],[],200);
    }
